...
## Figure 2: Disease-free survival and overall survival in the intention-to-treat population
...
## Figure 3: Cumulative incidence of locoregional recurrences and cumulative incidence of distant recurrences in the intention-to-treat population

The original Figure 3 in the paper shows the cumulative incidence functions for locoregional recurrence and distant metastases by treatment group. However, in our reproduction, we do not have the appropriate data to reproduce the cumulative incidence of locoregional recurrence. (Or we should do that too, and say it's not reproducible?)
...
## Figure 4: Disease-free survival in the intention-to-treat population by patient subgroups according to pretreatment and surgical or pathological factors after preoperative chemoradiotherapy

In Figure 4, we reproduced the forest plot for DFS hazard ratios with 95% confidence intervals across different subgroups, including age, sex, ECOG performance status, tumour location, T and N categories, ypT and ypN categories, resection completeness, pathological stage, and type of surgery.
...
Our reproduced forest plot is identical to the original figure reported by Rödel et al. (2015), with only minor differences (within ±0.01) in some subgroup hazard ratios. Notably, the hazard ratio differs by 0.09 in the ypTis/T1 subgroup.
...
forestplot(results.plot, labeltext = list("Subgroups", "Intervention (event/total n)", "Control (event/total n", "HR (95%CI)"), exp(est$m), exp(est$l), exp(est$u), zero = 1,
...
The primary endpoint analysis consists of stratified log-rank tests and mixed-effects Cox proportional hazards models to evaluate the effect of adding oxaliplatin to the standard 5-FU regimen on disease-free survival (DFS).

**Reproducibility Note**: All code for statistical analyses in this section is documented with step-by-step comments explaining the methodology, assumptions, and interpretation. Code is hidden from the output but fully documented for transparency and replication.

### Stratified Log-rank Test

We performed a stratified log-rank test accounting for the randomization stratification factors (center and clinical N category). Small centers with fewer than 4 patients were merged to ensure adequate power.

```{r}
#| label: stratified-logrank
#| echo: false

# Load required packages for primary endpoint analysis
# coin: for stratified log-rank tests with permutation distributions
# coxme: for mixed-effects Cox proportional hazards models
library(coin)
library(coxme)

# STEP 1: Handle small strata to ensure statistical power
# Check distribution of patients across centers (s_nz variable)
# Small centers with <4 patients need to be merged to avoid sparse data issues
strata_sizes <- table(CAOsurv$s_nz)
small_centres <- names(strata_sizes)[strata_sizes < 4]

cat(sprintf("Centres with < 4 patients: %d out of %d centres\n", 
            length(small_centres), length(strata_sizes)))

# Create merged strata variable for small centres
# This preserves the stratification structure while ensuring adequate sample sizes
CAOsurv$s_nzsmall <- as.character(CAOsurv$s_nz)
CAOsurv$s_nzsmall[CAOsurv$s_nz %in% small_centres] <- "small_centres_merged"
CAOsurv$s_nzsmall <- as.factor(CAOsurv$s_nzsmall)

cat(sprintf("After merging small centres: %d strata\n", nlevels(CAOsurv$s_nzsmall)))
...
# Display results
cat("Stratified Log-rank Test Results:\n")
cat(sprintf("Test statistic: %.4f\n", statistic(lr_test)))
cat(sprintf("P-value (approximate): %.6f\n", pvalue(lr_test)))
```
...
cat("Fitting mixed-effects Cox model with random intercepts...\n")

# STEP 1: Create stratification variables for random effects
# The model includes random intercepts to account for:
# - Center effects (s_nz): different centers may have varying baseline hazards
# - Clinical N category effects (gesamt_n_col): baseline prognostic factor
CAOsurv$strata_combined <- interaction(CAOsurv$s_nz, CAOsurv$gesamt_n_col, drop = TRUE)

# STEP 2: Fit mixed-effects Cox proportional hazards model
# Fixed effect: treatment arm (randarm)
# Random effects: center (s_nz) and clinical N category (gesamt_n_col)
# This accounts for clustering within centers and baseline prognostic factors
cox_me_model <- coxme(Surv(DFStime, DFSevent) ~ randarm + (1 | s_nz) + (1 | gesamt_n_col), 
                      data = CAOsurv)

cat("Mixed-effects Cox Model Summary:\n")
print(cox_me_model)
...
cat(sprintf("Hazard Ratio (5-FU + Oxaliplatin vs 5-FU): %.3f\n", hr))
...
cat("Generating Kaplan-Meier curves for proportional hazards assessment...\n")
...
km_fit <- prodlim(Surv(DFStime / 365, DFSevent) ~ randarm_plot, data = CAOsurv)
...
surv_5fu <- predict(km_fit, times = time_points, 
                    newdata = data.frame(randarm_plot = "5-FU"))[[1]]
surv_5fuox <- predict(km_fit, times = time_points, 
...
valid_idx_5fu <- surv_5fu > 0 & surv_5fu < 1
valid_idx_5fuox <- surv_5fuox > 0 & surv_5fuox < 1

loglog_5fu <- log(-log(surv_5fu[valid_idx_5fu]))
loglog_5fuox <- log(-log(surv_5fuox[valid_idx_5fuox]))
time_5fu <- time_points[valid_idx_5fu]
time_5fuox <- time_points[valid_idx_5fuox]

cat("Log-log transformation completed for proportional hazards assessment.\n")
```

### Survival Analysis Summary

We calculated survival rates at key time points (3-year and 5-year) for both treatment groups.
...
cat("3-year Disease-Free Survival:\n")
surv_3yr_5fu <- predict(km_fit, times = 3, 
                        newdata = data.frame(randarm_plot = "5-FU"))[[1]] * 100
surv_3yr_5fuox <- predict(km_fit, times = 3, 
                          newdata = data.frame(randarm_plot = "5-FU + Oxaliplatin"))[[1]] * 100

cat(sprintf("5-FU: %.1f%%\n", surv_3yr_5fu))
cat(sprintf("5-FU + Oxaliplatin: %.1f%%\n", surv_3yr_5fuox))

cat("\n5-year Disease-Free Survival:\n")
surv_5yr_5fu <- predict(km_fit, times = 5, 
                        newdata = data.frame(randarm_plot = "5-FU"))[[1]] * 100
surv_5yr_5fuox <- predict(km_fit, times = 5, 
                          newdata = data.frame(randarm_plot = "5-FU + Oxaliplatin"))[[1]] * 100
...
### Primary Endpoint Analysis Plots

#### Log-log Plot for Proportional Hazards

```{r}
#| label: fig-loglog-plot
#| fig-cap: Log-Log Plot for Proportional Hazards Assessment
#| echo: false
...
plot(time_5fuox, loglog_5fuox, type = "l", col = "red", lwd = 2,
     xlab = "Time (years)", ylab = "log(-log(S(t)))",
...
```{r}
#| label: fig-event-distribution
#| fig-cap: DFS Events by Treatment Arm
#| echo: false

# Create bar plot showing distribution of DFS events by treatment arm
# This visualization shows the number of events vs. censored observations
# Helps assess balance between treatment groups in terms of event rates
event_table <- table(CAOsurv$randarm, CAOsurv$DFSevent)
barplot(event_table, beside = TRUE, 
        main = "DFS Events by Treatment Arm",
        xlab = "DFS Event", ylab = "Number of Patients",
        col = c("lightblue", "lightcoral"),
        legend.text = rownames(event_table))
```

#### Follow-up Time Distribution

```{r}
#| label: fig-followup-distribution
#| fig-cap: Follow-up Time Distribution by Treatment Arm
#| echo: false

# Create boxplot showing distribution of follow-up times by treatment arm
# This visualization helps assess balance in follow-up duration between groups
# Important for ensuring comparability of survival analysis results
boxplot(CAOsurv$DFStime/365 ~ CAOsurv$randarm,
        main = "Follow-up Time Distribution",
        xlab = "Treatment Arm", ylab = "Time (years)",
        col = c("lightblue", "lightcoral"))
```
...
# COMPREHENSIVE SUMMARY OF PRIMARY ENDPOINT ANALYSIS
# This section provides a complete summary of all key statistical findings
# for the primary endpoint (disease-free survival) analysis

cat("PRIMARY ENDPOINT ANALYSIS - DISEASE-FREE SURVIVAL\n")
cat(paste(rep("-", 50), collapse=""), "\n", sep="")

# STEP 1: Display study population characteristics
cat(sprintf("Study Population: %d patients\n", nrow(CAOsurv)))
cat(sprintf("- 5-FU: %d patients\n", sum(CAOsurv$randarm == "5-FU")))
cat(sprintf("- 5-FU + Oxaliplatin: %d patients\n", sum(CAOsurv$randarm == "5-FU + Oxaliplatin")))

# STEP 2: Overall event rate
cat(sprintf("\nEvents: %d (%.1f%%)\n", sum(CAOsurv$DFSevent), 
            100 * sum(CAOsurv$DFSevent) / nrow(CAOsurv)))

# STEP 3: Key survival rates at clinically relevant time points
cat("\nSURVIVAL RATES:\n")
cat(sprintf("3-year DFS: 5-FU %.1f%% vs 5-FU+Oxaliplatin %.1f%%\n", surv_3yr_5fu, surv_3yr_5fuox))
cat(sprintf("5-year DFS: 5-FU %.1f%% vs 5-FU+Oxaliplatin %.1f%%\n", surv_5yr_5fu, surv_5yr_5fuox))

# STEP 4: Stratified log-rank test results
cat("\nSTRATIFIED LOG-RANK TEST:\n")
cat(sprintf("P-value: %.6f\n", pvalue(lr_test)))
cat(sprintf("Significance level: %s\n", ifelse(pvalue(lr_test) < 0.05, "Significant (p < 0.05)", "Not significant (p >= 0.05)")))

# STEP 5: Mixed-effects Cox model results (accounts for clustering)
cat("\nMIXED-EFFECTS COX MODEL:\n")
cat(sprintf("Hazard Ratio: %.3f (95%% CI: %.3f - %.3f)\n", hr, hr_ci_lower, hr_ci_upper))
cat(sprintf("P-value: %.6f\n", p_value_cox))
cat(sprintf("Interpretation: 5-FU + Oxaliplatin %s the hazard by %.1f%% compared to 5-FU alone\n",
            ifelse(hr < 1, "reduces", "increases"),
            abs((hr - 1) * 100)))

# STEP 6: Model assumptions assessment guidance
cat("\nPROPORTIONAL HAZARDS ASSUMPTION:\n")
cat("Check the log-log plot for parallel lines. If lines are approximately parallel, the proportional hazards assumption is reasonable.\n")
```
