...
# Check for small strata and create merged version if needed
...
# Create merged strata for small centres
...
# Stratified log-rank test using coin package
set.seed(29)  # For reproducibility of permutation test
...
# Fit mixed-effects Cox model with random intercepts for centre and cN category
cat("Fitting mixed-effects Cox model with random intercepts...\n")

# Create combined stratification variable for random effects
...
# Extract hazard ratio and confidence interval
hr <- exp(coef(cox_me_model))
hr_se <- sqrt(vcov(cox_me_model)[1,1])
hr_ci_lower <- exp(coef(cox_me_model) - 1.96 * hr_se)
hr_ci_upper <- exp(coef(cox_me_model) + 1.96 * hr_se)
p_value_cox <- 2 * pnorm(-abs(coef(cox_me_model) / hr_se))
...
# Kaplan-Meier curves for log-log plot
...
# Create time points for log-log plot
...
# Calculate log-log values (remove zeros to avoid -Inf)
