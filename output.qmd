...
The lines crossed early in the follow-up time, so we think there is evidence on violation of the proportional hazard (PH) assumption. We notice that during the majority of follow-up period, the PH assumption is met. We are how sure how the violation of the PH assumption at early stage affects the model.  
...
We found that the numbers at risk at year 1, 3, and 5 have one person off in control group, given by the `survminer` package. In order to check the reason, we use an alternative way to plot the cumulative incidence using the outcome variable with competing risks via the `prodlim` package.
...
The numbers at risk match with the original paper in our manual calculation. Based on this exploration, we found that both `survminer` and `prodlim` package includes participants who experienced events at each time mark in the number at risk count.
...
cm_os <- coxme(Surv(OStime, OSevent) ~ randarm + (1 | s_nzsmall), 
                      data = CAOsurv)
...
cm_os_fit <- prodlim(Hist(OStime / 365, OSevent) ~ randarm_plot + cluster(s_nzsmall), data = CAOsurv)
