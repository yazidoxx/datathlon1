...
The probability of DFS at 3 years is `r round(surv_3yr_5fuox,2)`% in the investigational group and `r round(surv_3yr_5fu,2)`% in the control group. **These probabilities match the numbers reported in the paper.**
...
prepare_subgroup_analysis <- function() {
  ...
  calculate_hr <- function(data, condition = NULL, label) {
    ...
    cox_model <- tryCatch({
      ...
      coxme(Surv(dfs_time, dfs_status) ~ treatment + (1|s_nzsmall), data = subgroup_data)
    }, error = function(e) {
      return(NULL)
    })
