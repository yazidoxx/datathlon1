...
The reproduced hazard Ratio for disease-free survival (5-FU + Oxaliplatin vs 5-FU) is `r round(hr, 3)` with 95% CI: (`r round(hr_ci_lower,3)`, `r round(hr_ci_upper,3)`), p=`r round(p_value_cox, 3)`.
...
km_fit2 <- prodlim(Hist(DFStime / 365, DFSevent) ~ randarm_plot + cluster(s_nzsmall), data = CAOsurv)
...
\[\[Maybe it's a violation...\]\] Although the lines crossed early in the follow-up time, there is no evidence on the violation of proportional hazard assumption during the major follow-up period.
...
surv_3yr_5fu <- predict(km_fit2, times = 3, 
                        newdata = data.frame(randarm_plot = "5-FU"))[[1]] * 100
surv_3yr_5fuox <- predict(km_fit2, times = 3, 
...
The probability of DFS at 3 years is `r surv_3yr_5fuox`% in the investigational group and `r surv_3yr_5fu`% in the control group. 
