...
**Additional analysis** was performed to extend the original analysis by applying the same mixed-effects Cox model framework to overall survival and the proportional hazards assumption was checked using log–log survival plots. 
...
cuminc(Surv(lrtime, LRplot_f) ~ randarm, df.fig3) %>%
  ggcuminc(outcome = "locoregional recurrence") +
...
cuminc(Surv(drtime, DRcmptrsk) ~ randarm, df.fig3) %>%
  ggcuminc(outcome = "distant recurrence") +
  add_confidence_interval() +
  add_risktable() +
...
The numbers at risk in our manual calculation match with the reported numbers in the paper. Based on these explorations, we found that *`survminer`, `tidycmprsk` and `prodlim` package includes participants who experienced events at each time mark in the number at risk count.*
...
**Using only ypT1, we found the HR as presented in Figure 4 in the paper which is `r round(hr_ypt,2)` (95% CI: `r round(hr_loci_ypt,2)`, , `r round(hr_upci_ypt,2)`).**
